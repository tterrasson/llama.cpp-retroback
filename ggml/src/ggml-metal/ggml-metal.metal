...
// retro delta: SILU backward for LoRA training.
// dst = dy * s * (1 + x*(1 - s)), with s = sigmoid(x).
// src[0] = g = dy (grad of output), src[1] = x (forward input). Contiguous F32.
kernel void kernel_silu_back_f32(
    ...
    dst[gid] = g[gid] * s * (1.0f + xi * (1.0f - s));
}
