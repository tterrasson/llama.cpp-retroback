...
// retro delta: the list of RIR shader modules, generated by rir-gen and copied
// here byte for byte (docs/FUTURE_V1.md §15).
#include "rir_vk_artifacts.h"
...
void process_shaders() {
    ...
    // retro delta: `rms_norm_back_f32` and `l2_norm_back_f32` stood above and
    // beside this line. Both natives are retired (docs/INT_RIR_V4.md §P6); the
    // `rir_` variants below are the only implementation of those two ops.
    ...
    //
    // The list is generated too (docs/FUTURE_V1.md §15). It used to be one
    // hand-written line per *variant* here — sixty-six of them, growing by one
    // every time a kernel gained a lowering, in a vendored file that every
    // rebase has to merge. What a RIR module needs is uniform (no #define, the
    // file name is the artifact name, the module name is that name prefixed),
    // so the whole of it fits in this expansion and the list itself is data.
#define RIR_STRING_TO_SPV(name) string_to_spv("rir_" #name, "rir_" #name ".comp", {});
    RIR_VK_ARTIFACTS(RIR_STRING_TO_SPV)
#undef RIR_STRING_TO_SPV
