...
// retro delta: the RIR shaders declare themselves in a header of their own
// (docs/INT_RIR_V3.md §R0). Adding a generated kernel then touches only the
// small translation unit that resolves a blob by name, instead of the 20 000
// lines of ggml-vulkan.cpp — 6 of the 9 min 30 a `--build` used to cost, for
// two `extern` lines of wiring.
std::string target_rir_hpp = "";

// retro delta: a generated shader is exactly one whose name carries the RIR
// prefix; the generator names them nowhere else, so this is the whole rule.
static bool is_rir_shader(const std::string & name) {
    return name.rfind("rir_", 0) == 0;
}
...
void process_shaders() {
    ...
    string_to_spv("rir_out_prod", "rir_out_prod.comp", {});
    string_to_spv("rir_rms_norm_back", "rir_rms_norm_back.comp", {});
    string_to_spv("rir_rms_norm_back_shared_reduce", "rir_rms_norm_back_shared_reduce.comp", {});
    string_to_spv("rir_add", "rir_add.comp", {});
    string_to_spv("rir_mul", "rir_mul.comp", {});
    string_to_spv("rir_scale", "rir_scale.comp", {});
    string_to_spv("rir_add_vec4", "rir_add_vec4.comp", {});
    string_to_spv("rir_mul_vec4", "rir_mul_vec4.comp", {});
    ...
    // retro delta: fused sparse cross-entropy (see docs/memory/GPUVOCAB.md).
...
void write_output_files() {
    ...
    // retro delta: the RIR shaders' own header, and the X-macro list that lets
    // one small translation unit resolve a blob by name without restating what
    // shaders exist (docs/INT_RIR_V3.md §R0).
    std::stringstream rir_hdr = make_generic_stringstream();
    std::vector<std::string> rir_names;

    hdr << "#include <cstdint>\n\n";
    rir_hdr << "// Generated by vulkan-shaders-gen — DO NOT EDIT MANUALLY.\n";
    rir_hdr << "// SPIR-V of the RIR-generated shaders, kept out of "
            << basename(target_hpp) << " so that\n";
    rir_hdr << "// adding a generated kernel does not recompile ggml-vulkan.cpp "
               "(docs/INT_RIR_V3.md §R0).\n";
    rir_hdr << "#include <cstdint>\n\n";
    // A `.cpp` holds one shader source's blobs, so it includes the header that
    // declares them — the RIR one for a RIR source, the general one otherwise.
    const bool cpp_is_rir = input_filepath != "" && is_rir_shader(basename(input_filepath));
    src << "#include \"" << basename(cpp_is_rir ? target_rir_hpp : target_hpp) << "\"\n\n";
    ...
    for (const auto& pair : shader_fnames) {
        ...
        // retro delta: a RIR shader declares itself in the RIR header only.
        std::stringstream & decl = is_rir_shader(name) ? rir_hdr : hdr;
        if (is_rir_shader(name)) {
            rir_names.push_back(name);
        }
        decl << "extern const uint64_t " << name << "_len;\n";
        decl << "extern const unsigned char " << name << "_data[];\n\n";
    ...
    if (input_filepath == "") {
        ...
        // retro delta: the list itself, so the resolving unit enumerates what
        // the generator emitted rather than a copy someone keeps in step.
        rir_hdr << "#define RIR_VK_SHADERS(X) \\\n";
        for (size_t i = 0; i < rir_names.size(); ++i) {
            rir_hdr << "    X(" << rir_names[i] << ")"
                    << (i + 1 < rir_names.size() ? " \\" : "") << "\n";
        }
        if (rir_names.empty()) {
            rir_hdr << "    /* no RIR shader in this build */\n";
        }
        if (target_rir_hpp != "") {
            write_file_if_changed(target_rir_hpp, rir_hdr.str());
        }
    }
    if (target_cpp != "") {
...
int main(int argc, char** argv) {
    ...
    // retro delta: separate header for the RIR-generated shaders
    if (args.find("--target-rir-hpp") != args.end()) {
        target_rir_hpp = args["--target-rir-hpp"];
    }
